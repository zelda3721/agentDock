...
> **方向变更（2026-07-20，用户裁决）：聊天界面采「混合现代（方向 B）」。**
> 用户在三方向原型对比后选定 B：**助手回答保持全宽**（Claude/ChatGPT 式，仪表识别度不丢）；
> **用户消息改为右对齐柔角卡片**（中性面 `surface_sunken` + `radius_lg`，**不蹭青琥珀语义轴**）；
> 助手眉标加**来源头像**（小圆 + 语义色底 + 坞形/云弧图元，是来源冗余的一环）；
> **输入区圆形发送键**（青底 + 上箭头）+ `radius_lg` 输入井。
> 本节及下列条目中与之相抵的部分（[R-38a] 禁止三件套、[R-72] 用户消息不得右对齐、§3 B4 非气泡）
> **就此让位**——保留原文与理由供溯源，但聊天界面以方向 B 为准。**溯源条 / ctx 水位 / 语义轴 /
> tok·s 等仪表识别度仍全部保留**，这是 B 相对「消费级气泡（方向 C）」被选中的前提。

1. ~~**不做通用聊天 App 外观**（左右气泡 + 圆头像 + 大发送按钮那一套）。~~ **（聊天界面已让位方向 B，见上方变更框；仪表母题仍在。）** AgentDock 的世界是「坞站与仪器」：本地 CPU 正在跑模型，有 tok/s、首 token 时延、内存预算、KV 缓存、召回率、置信度、token 预算水位——**这些数字是这个产品的材质**，不是调试信息。视觉母题 = 精密仪器 + 鸿蒙的柔和克制。不是赛博霓虹，不是玻璃拟态堆砌。
...
- **ctx 预算水位线**（输入框上沿一条 2vp 细线，随上下文占用变化；压缩发生时显示"整理思路中"）。
  混合现代方向 B（2026-07-21）：**「ctx N%」数值读数长在输入胶囊内右侧**（效果图形态），线仍在上沿；
  读数只在 tokenizer 给出真值后上屏（不画 0% 的假表），阈值配色与线同源（[R-18]）。
...
> **[R-72] 的一个隐含推论**：~~**用户消息不得右对齐。**~~ **（已让位方向 B，2026-07-20：用户消息右对齐柔角卡片。）**
> 原顾虑仍如实记录：右对齐后用户卡与助手溯源条不在同一左基线。方向 B 接受这一取舍——
> **溯源条只服务助手消息**（用户轮次本就无溯源），故它仍与助手正文左对齐、自成一列；用户卡靠右不与之竞争基线。
> [R-72] 对**助手**侧「溯源条与正文同左基线」的要求不变。
...
**用户轮次的形态（[R-38a]）** —— **已让位方向 B（2026-07-20 用户裁决）。** 现行形态：
- **用户消息**：右对齐柔角卡片，`max-width` 82% + 中性 `surface_sunken` 底 + `radius_lg` + hairline 描边（**不用青琥珀**，避免蹭语义轴）。
- **助手消息**：全宽通栏 + 左 3vp 语义色带（来源断言）+ **组首眉标「[头像] 智能体名 · 时间戳」**
  （头像 = 会话绑定智能体的 emoji，经 conversations⋈agent_defs.avatar 接进 message VM；缺则回退
  来源色坞形/云弧 SVG 图元——仍是来源冗余 [R-32]）+ 溯源条脚注。眉标显隐随组首（角色切换）。
- **发送键**：`AdIconButton` 圆形（`SOLID` 青底 + `ARROW_UP` SVG 上箭头），`visualSize` 40。停止键仍用文字「停止」（danger），语义更清楚。

~~**禁止三件套**~~ **（下列三条在方向 B 下解除，仅作历史记录）**：① 右对齐气泡；② 圆头像（`radius_full`）；③ 青色圆形发送键。
**原理由（保留供溯源）**：这三件套齐了偏「通用聊天 App」。方向 B 用**保留全套仪表识别度（溯源条 / ctx 水位 / 语义轴 / 来源头像仍是 SVG 图元而非 emoji）** 来守住 §1.2 的立场——不是把仪表也一起丢了（那是方向 C，已被否）。
...
- [ ] 聊天界面采**方向 B（混合现代，2026-07-20）**：用户右卡片 + 助手全宽 + 来源头像（SVG 非 emoji）+ 圆形发送键；**仪表识别度（溯源条 / ctx 水位 / 语义轴）全保留**（这是 B 相对 C 被选中的前提）

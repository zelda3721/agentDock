...
- **助手消息**：全宽通栏 + 左 3vp 语义色带（来源断言）+ 溯源条脚注（身份由这两者承担，助手 `showHeader` 恒 false，无眉标）。**AI 头像 + 智能体名**是方向 B 的后续增强（需把 agent avatar/name 接进 message VM；当前 roleLabel 只有通用「智能体」，不宜直接展示）——见 backlog。

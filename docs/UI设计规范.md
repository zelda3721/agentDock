...
**lg 断点的第三栏就是它的展开态（检视栏 / Inspector）**——不是"详情面板"的泛称，只承载引用块 / 记忆条 / 被围栏拦截项三类内容（Run 轨迹与检索调试器复用同一栏位，顶部有明确 Tab 标识）。md/sm 降级为底部 Sheet，**内容完全相同，只换容器**。
...
展开态 · lg：不就地展开，内容推入第三栏检视栏（C3 AdInspector）；
...
**ArkUI**：`@ComponentV2` + `@Param provenance`；点亮用 `animateTo` + 逐段 `@Local` opacity 数组 + 60ms 延时链；lg 检视栏联动经既有 `EventBus`（core-infra）广播 `inspector.show`，**不引全局单例状态**。
...
**ArkUI**：`Tabs`（sm）/ `SideBarContainer`（md/lg）；断点来自既有 `BreakpointSystem.ets`（`@StorageProp('currentBreakpoint')`）——**禁止各页面自行监听 mediaquery**。
...
└──────────────────────────────┘
```
**内容来源**：点击 `AdProvenanceRail` 任一字段 → 检视栏滚到对应分组并高亮（`dur_fast` 底色闪一次）。
**[R-70] 降级路径（强制）**：md/sm 无第三栏 → 同一内容渲染进 `AdSheet`（底部弹出）；840–1232vp 窗口 → 右侧 overlay。**组件内容完全相同，只换容器**——这是"一次开发多端部署"的具体含义，**不允许两套内容**。**检视栏在任何降级下都不许消失**，入口（溯源条点按 / `Ctrl+I`）必须恒在。
